...
#ifdef NONBLOCKING_BST_ARRAYBASED

#define ENABLE_DATA_PARTITION
#define ENABLE_REVALIDATION

#include "bst_arraybased_header.h"
...
int final_count;
    ...
    // #include "multiple_snapshot_util.h"
...
bool BST<T>::insert(T newItem) {
       ...
       WRITE_OP_BEGIN((newItem/partition_size))
         ...
         // root_node = tmp_var_0;
         root_node = root[snapshot_idx]->value;
         Node* tmp_var_1;
...
/*
template<class T> GC_Queue<typename BST<T>::synobj_t>* BST<T>::GarbageCollector::synobj_queue[GC_MAX_THREADS];
template<class T> GC_Queue<typename BST<T>::wrapper_t>* BST<T>::GarbageCollector::wrapper_queue[GC_MAX_THREADS];
template<class T> GC_Queue<typename BST<T>::gc_node_t>* BST<T>::GarbageCollector::node_queue[GC_MAX_THREADS];
...
template<class T> unsigned BST<T>::ConflictArray::shift = 0;
template<class T> unsigned BST<T>::ConflictArray::mask = 0;
template<class T> GC_Array_Allocator<volatile int> BST<T>::ActivityArray::allocator;
template<class T> GC_Array_Allocator<volatile void*> BST<T>::ConflictArray::allocator;
template<class T> GC_Array_Allocator<typename BST<T>::GC_Retired_SynObj> BST<T>::RetiredSynObjArray::allocator;
*/

GC_STATIC_SETUP(template<class T>, BST<T>);
...
#endif // NONBLOCKING_BST_ARRAYBASED
