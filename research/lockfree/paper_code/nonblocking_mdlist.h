...
#define GC_BLOCK_SIZE       (16*800000/NUM_OF_THREADS)
// #define GC_BLOCK_SIZE       (100000)
#define GC_RESERVE_SIZE     (64)
#define LOG_NUM             16
#define GC_NODE_BLOCK_SIZE       (300000)
#define READ_NUM            80
#define NODE_NUM            1
...
MDList<T>::MDList ()  {
   ...
   m_head = new Node ();
   std::memset(m_head,0,sizeof(Node));
   // write_wrapper(m_head->m_child[0]) = new Node();
   m_head->m_child[0] = new wrapper_t;
   m_head->m_child[0]->value = new Node();
   // std::memset((write_wrapper(m_head->m_child[0])),0,sizeof(Node));
   std::memset(m_head->m_child[0]->value,0,sizeof(Node));
   // write_wrapper(m_head->m_child[0])->m_key = -1;
   m_head->m_child[0]->value->m_key = -1;
   state = state_tmp;
}
...
#ifdef CHECK_CORRECTNESS
    final_count = 0;
    Traverse(read_wrapper(m_head->m_child[0]),NULL,0);
...
bool MDList<T>::Insert(uint32_t key) {
  ...
  WRITE_OP_BEGIN
     ...
     pred = NULL;
     Node* tmp_var_0;
     // MOD_READ(m_head->m_child[0],tmp_var_0);
     tmp_var_0 = m_head->m_child[0]->value;
     curr = tmp_var_0;
     dim = 0;
     pred_dim = 0;
...
bool MDList<T>::Delete(uint32_t key) {
  ...
  WRITE_OP_BEGIN
     ...
     pred = NULL;
     Node* tmp_var_3;
     // MOD_READ(m_head->m_child[0],tmp_var_3);
     tmp_var_3 = m_head->m_child[0]->value;
     curr = tmp_var_3;
     dim = 0;
     pred_dim = 0;
     while (dim<DIMENSION)
...
bool MDList<T>::Find(uint32_t key) {
   ...
   READ_OP_BEGIN
     pred = NULL;
     Node* tmp_var_8;
     // MOD_READ_R(m_head->m_child[0],tmp_var_8);
     tmp_var_8 = m_head->m_child[0]->value;
     curr = tmp_var_8;
     dim = 0;
     pred_dim = 0;
     while (dim<DIMENSION)
