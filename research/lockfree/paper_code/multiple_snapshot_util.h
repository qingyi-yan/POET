...
struct ConflictArray {
    ...
    void publish (void *item, int idx) {
        // assert(idx < CONFLICT_ARRAY_SIZE);
        ringbuffer[idx] = item;
    }
};
...
struct GarbageCollector {
    ...
    static GC_Wrapper_Queue<wrapper_t>* wrapper_to_reuse[GC_MAX_THREADS];
#ifdef ENABLE_GC_NODE_ALLOCATOR
    static GC_Node_Queue<gc_node_t>* node_to_reuse[GC_MAX_THREADS];
#endif
    ...
    GarbageCollector () {
      ...
      for (int i = 0; i < GC_MAX_THREADS; i++) {
        obj_to_reuse[i] = new GC_SynObj_Queue<synobj_t>(this);
      }
    }

    inline int get_gc_idx () {
        printf("shouldn't call this function");
        exit(0);
      return local_gc_idx;
    }
};
...
inline int get_gc_idx() {
  if (local_gc_idx == -1) {
    ...
    GarbageCollector::read_obj[local_idx]->synobj.parent_gc = gc_array[local_idx];
    GarbageCollector::wrapper_to_reuse[local_idx] = new GC_Wrapper_Queue<wrapper_t>();
#ifdef ENABLE_GC_NODE_ALLOCATOR
    GarbageCollector::node_to_reuse[local_idx] = new GC_Node_Queue<gc_node_t>();
#endif
...
#define WRITE_OP_END        if (!modified) break; \
                            if (!state_array[snapshot_idx].compare_exchange_weak(oldstate,newstate)) {\
                                while(true) { \
                                    ...
                                    if (state_array[snapshot_idx].compare_exchange_weak(oldstate,newstate)) { retire_synobj(newstate); break;}\
                                    nanosleep_invoke(); \
                                } \
                            } \
                            retire_synobj(newstate); newstate->apply_mod(); break; \
...
#define WRITE_OP_END        if (!modified) break;\
                            if (state_array[snapshot_idx].compare_exchange_weak(oldstate,newstate)) {\
                                retire_synobj(newstate); newstate->apply_mod(); \
                                break; \
                            } \
