...
struct Template_Synobj
{
    ...
    inline void apply_mod () {
        ...
        for (int j = 0; j < log_num; j++) {
            int i = (j + local_gc_idx)%total_log_num;
            wrapper_type** addr = logs[i].addr;
            if (addr == 0) continue;
...
struct GC_SynObj_Queue {
    ...
    struct GC_SynObj_Block {
        ...
        GC_SynObj_Block() {
            elems = new U [GC_SYNOBJ_BLOCK_SIZE];
            newest_version = -1;
        }
    };
    ...
    GC_SynObj_Queue (GarbageCollector *p_gc) {
        ...
        for (int i = 1; i < GC_RESERVE_SIZE; i++) {
    ...
    inline U* pop() {
        if (unlikely(block_idx == GC_SYNOBJ_BLOCK_SIZE)) {
...
inline synobj_t *allocate_synobj() {
  if (last_state != NULL) return (synobj_t*) last_state;
  int gc_idx = gc->get_gc_idx();
  gc->new_obj[gc_idx] = gc->allocate_synobj(gc_idx);
  last_state = gc->new_obj[gc_idx];
...
inline void retire_synobj (synobj_t *x) {
  last_state = 0;
  return; // no need to reclaim
  // int gc_idx = gc->get_gc_idx();
  // gc->obj_to_reuse[local_gc_idx]->push(x);
}
