...
void MDList::Insert (int key) {
   ...
   WRITE_OP_BEGIN(key%size_subtree)
     ...
     pred = NULL;
     Node* tmp_var_0;
     SNAPSHOT_READ(m_head_entry,tmp_var_0);
     curr = tmp_var_0;
     dim = 0;
     pred_dim = 0;
     while (dim<DIMENSION)
          ...
          if (pred!=NULL) 
            {
               ...
               MOD_WRITE(pred->m_child[pred_dim],pred_m_child_pred_dim_copy);
               {
                  for (int i=pred_dim; i<dim; i+=1)
                    {
                       ...
                       MOD_WRITE(curr->m_child[i],curr_m_child_i_copy);
            ...
            {
               m_head_key_m_child__copy = new_node;
               MOD_WRITE(m_head[key%size_subtree]->m_child[0],m_head_key_m_child__copy);
            }
       }
...
bool MDList::Delete (int key) {
   ...
   WRITE_OP_BEGIN(key%size_subtree)
     ...
     pred = NULL;
     Node* tmp_var_3;
     SNAPSHOT_READ(m_head_entry,tmp_var_3);
     curr = tmp_var_3;
     dim = 0;
     pred_dim = 0;
     while (dim<DIMENSION)
     ...
     if (dim==DIMENSION) 
       {
          ...
          if (pred==NULL) 
            {
               m_head_key_m_child__copy = nextChild;
               MOD_WRITE(m_head[key%size_subtree]->m_child[0],m_head_key_m_child__copy);
            }
          else 
            {
               ...
               for (int i=pred_dim; i<child_dim; i+=1)
                 {
                    ...
                    nextChild_m_child_i_copy = tmp_var_7;
                    MOD_WRITE(nextChild->m_child[i],nextChild_m_child_i_copy);
                 }
            }
          __ret__10 = true;
       }
...
bool MDList::Find (int key) {
   ...
   int dim=0;
   int pred_dim=0;
   READ_OP_BEGIN(key%size_subtree)
     ...
     pred = NULL;
     Node* tmp_var_8;
     SNAPSHOT_READ(m_head_entry,tmp_var_8);
     curr = tmp_var_8;
     dim = 0;
     pred_dim = 0;
     while (dim<DIMENSION)
