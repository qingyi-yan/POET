   ...
   for (k=1; k<Expansion_Terms; k+=1) 
     {
        ...
        COMPLEX_MUL(temp_exp[k],z0_pow_minus_n[k],source_box->mp_expansion[k]);
     }
   
   for (k=0; k<Expansion_Terms; k+=1) 
     {
        result_exp.r = (real)0.0;
        result_exp.i = (real)0.0;
        
        for (j=1; j<Expansion_Terms; j+=1) 
          {
             {
                 if (((j & 0x1) == 0x0) == 
                     (((j+1) & 0x1) == 0x0)) {
                     printf("[Profile Conflict] Unroll branch with factor 2 for j\n");
                     exit(-1);
                 }
             }
             temp.r = C[k+j-1][j-1];
...
void main() {
   ...
   printf("[Unroll Branch] The loop can be unrolled to eliminate branch: j\n");
   printf("\n=============================================\n\n\n");
}
