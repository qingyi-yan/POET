        ...
        /*@;BEGIN(Nest2=Nest)@*/for (iindex=firstcol; iindex<1+lastcol; iindex+=1)
          {
             ...
             f3 = (y[i+1][iindex]+y[i+1][iindex+1]-y[i-1][iindex]-y[i-1][iindex+1])*(x[i][iindex+1]-x[i][iindex]);
             f4 = (y[i+1][iindex-1]+y[i+1][iindex]-y[i-1][iindex-1]-y[i-1][iindex])*(x[i][iindex]-x[i][iindex-1]);
