...
void relax(long k, double *err, long color, long my_num)
{
   ...
   *err = 0.0;
   ...
   maxerr = 0.0;
   if (color == RED_ITER) {
   /*@; BEGIN(Nest1=Nest) @*/for (i=evenistart;i<iend;i+=2) {
   /*@; BEGIN(Nest2=Nest) @*/  for (j=evenjstart;j<jend;j+=2) {
     ...
     /*@; BEGIN(Nest3=Nest) @*/for (i=oddistart;i<iend;i+=2) {
     /*@; BEGIN(Nest4=Nest) @*/  for (j=oddjstart;j<jend;j+=2) {
         a = multi->q_multi[k][i][j+1] + multi->q_multi[k][i][j-1] +
	     multi->q_multi[k][i-1][j] + multi->q_multi[k][i+1][j] -
	     multi->rhs_multi[k][i][j] ;
         oldval = multi->q_multi[k][i][j];
     ...
     /*@; BEGIN(Nest5=Nest) @*/for (i=evenistart;i<iend;i+=2) {
     /*@; BEGIN(Nest6=Nest) @*/  for (j=oddjstart;j<jend;j+=2) {
         a = multi->q_multi[k][i][j+1] + multi->q_multi[k][i][j-1] +
	     multi->q_multi[k][i-1][j] + multi->q_multi[k][i+1][j] -
	     multi->rhs_multi[k][i][j] ;
         oldval = multi->q_multi[k][i][j];
     ...
     /*@; BEGIN(Nest7=Nest) @*/for (i=oddistart;i<iend;i+=2) {
     /*@; BEGIN(Nest8=Nest) @*/  for (j=evenjstart;j<jend;j+=2) {
         a = multi->q_multi[k][i][j+1] + multi->q_multi[k][i][j-1] +
	     multi->q_multi[k][i-1][j] + multi->q_multi[k][i+1][j] -
	     multi->rhs_multi[k][i][j] ;
         oldval = multi->q_multi[k][i][j];
