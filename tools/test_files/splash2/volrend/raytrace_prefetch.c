...
unsigned char _shift_array_0[]= { 0x1,0x2,0x4,0x8,0x10,0x20,0x40,0x80 };
void Trace_Ray(float foutx,float fouty,PIXEL* pixel_address) {
   ...
   float yalpha;
   float zalpha;
   float one_minus_xalpha;
   float one_minus_yalpha;
   float one_minus_zalpha;
   float weight;
   float wopacity;
   float wopacitysum;
   float wcolorsum;
   ...
   for (i=0; i<NM; i+=1) 
     {
        float _subexpression_var_0=((float)(opc_len[i]-1));
        if (ABS(ray[1][i]-ray[0][i])<SMALL)  
          {
             if ((ray[0][i]<_float_constant_0&&ray[1][i]<_float_constant_0)||(ray[0][i]>_subexpression_var_0&&ray[1][i]>_subexpression_var_0))  
               {
                  return ;
               }
             else 
        ...
        rmax = (_subexpression_var_0-ray[0][i])/(ray[1][i]-ray[0][i]);
        if (rmin<rmax)  
          {
   ...
   while (1)  
     {
/*@;BEGIN(Stmt1=ExpStmt)@*/pyr_offset1 = ((ivoxel[Z]>>level)*pyr_len[level][Y]+(ivoxel[Y]>>level))*pyr_len[level][X]+(ivoxel[X]>>level);
        pyr_offset2 = pyr_offset1&7;
        pyr_address2 = pyr_address[level]+(pyr_offset1>>3);
        bit = *pyr_address2&_shift_array_0[pyr_offset2];
...
/*@;BEGIN(Stmt2=ExpStmt)@*/pyr_offset1 = (samplez*pyr_len[0][Y]+sampley)*pyr_len[0][X]+samplex;
        pyr_offset2 = pyr_offset1&7;
