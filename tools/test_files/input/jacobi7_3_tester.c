...
void jacobi7_3_ref(const int nx,const int ny,int nz,const double alpha,double* A0,const int timesteps,const double* B,const int ldb,double* Anext,const int ldc) {
   double fac;
   double* temp_ptr;
   int i;
   int j;
...
int main(int argc, char **argv) 
{
  ...
  ny = SIZE;
  nx = SIZE;
  Anext_size=Index3D(nx,ny,nx-1,ny-1,nz-1);
  ...
  Anext_comp_size=Index3D(nx,ny,nx-1,ny-1,nz-1);
  ...
  jacobi7_3 (nx,ny,nz,alpha,A0_comp,timesteps,B,ldb,Anext_comp,ldc);
  
  jacobi7_3_ref (nx,ny,nz,alpha,A0,timesteps,B,ldb,Anext,ldc);
  ...
  for (__pt_i0=0; __pt_i0<Index3D(nx,ny,nx-1,ny-1,nz-1); ++__pt_i0)
  {
    if(Anext_comp_buf[__pt_i0] != Anext_buf[__pt_i0]) {
      ...
      printf("Position %d (%f) and Position %d (%f) differ by %.15f\n", __pt_i0,
      	...
      	fabs(Anext_comp_buf[__pt_i0]-Anext_buf[__pt_i0]));
    }
    /*else {
      printf("Identical output at index %d\n", __pt_i0);
    } */
