...
void jacobi7_4_ref(const int nx,const int ny,int nz,const double alpha,double * A0,const int timesteps,const double * B,const int ldb,double * Anext,const int ldc) 
{
   double fac;
   double * temp_ptr;
   int i,j,k,t;
...
int main(int argc, char **argv) 
{
  ...
  jacobi7_4 (nx,ny,nz,alpha,A0_comp,timesteps,B,ldb,Anext_comp,ldc);
  
  jacobi7_4_ref (nx,ny,nz,alpha,A0,timesteps,B,ldb,Anext,ldc);
  
  {
  int diff_flag = 0;
  for (__pt_i0=0; __pt_i0<Index3D(nx,ny,nx-1,ny-1,nz-1+1); ++__pt_i0)
  {
