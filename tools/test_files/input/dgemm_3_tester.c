...
/* routine to test*/
void dgemm_3(const int M,const int n,const int K,const double alpha,const double * a,const int lda,const double * b,const int ldb,const double beta,double * c,const int ldc) 
;
/* macro for the value of routine parameter */
#ifndef SIZE
#define SIZE 200
#endif
void dgemm_3_ref(const int M,const int n,const int K,const double alpha,const double * a,const int lda,const double * b,const int ldb,const double beta,double * c,const int ldc) 
{
   int i,j,k;
   for (j=0; j<n; j+=1) 
     {
...
int main(int argc, char **argv) 
{
  ...
  dgemm_3 (M,n,K,alpha,a,lda,b,ldb,beta,c_comp,ldc);
  
  dgemm_3_ref (M,n,K,alpha,a,lda,b,ldb,beta,c,ldc);
