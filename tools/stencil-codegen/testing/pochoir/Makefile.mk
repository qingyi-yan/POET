...
$(SRC).$(EXT_POCHOIR).c: $(SRC).cpp prepare
	pochoir -o $(STAGE_PATH)/$(SRC).$(gen_id) $(POCHOIRFLAGS) $(SRC)_$(gen_id).cpp
...
build:
	$(PCG) $(PCGFLAGS) $(DBG) -pdim=$(dim) -ptime=$(T_MAX) -poa=$(oa) -pto=$(to) -pdiv=$(div) -pmi=$(mi) ../../stencil-gen-pochoir.pt > out_$(gen_id).cpp
...
opt: prepare build $(SRC).$(OPT).c
	...
	$(eval EXE_OUT=x$(SRC)_$(CC)_$(gen_id))
	cp $(STAGE_PATH)/$(SRC).$(gen_id) $(OUTPUT_DIR)/$(EXE_OUT) 
	@if [ "$(TEST_MODE)" = "1" ]; then\
...
div_sweep:
	@$(MAKE) div=0 mi=0 to_sweep
	@$(MAKE) div=1 mi=0 to_sweep
	@$(MAKE) div=1 mi=1 to_sweep
	@$(MAKE) div=4 mi=0 to_sweep
	@$(MAKE) div=4 mi=1 to_sweep

opt_sweep:
	@$(MAKE) CC=gcc OPT=pochoir div_sweep
