...
int main(int argc, char ** argv) 
{
  ...
  Pochoir_Shape_1D stencil_shape[] = {{1,0}, {0,-1}, {0,1}, {0,0}};
  Pochoir_1D stencil(stencil_shape);

  Pochoir_Kernel_1D(stencil_fn,t,i0)
  u(t+1,i0) = ((((1 * u(t,i0-1) + -2 * u(t,i0) + 1 * u(t,i0+1)) / divvar) / divvar) / divvar) / divvar + 1*u(t,i0);
  Pochoir_Kernel_End
  ...
  benchEndStencil();
  benchSetEnv();
  benchSetDomain(1,2,1);
  benchSetProblemSize(N0, 0, 0, TS);
  benchSetArithProps(3, 0, 4, 4);
