...
if ((N0 >= 5) && (N1 >= 5) && (N2 >= 5) && (TS >= 1)) {
  for (t1=-1;t1<=floord(TS-1,2);t1++) {
    lbp=max(ceild(t1,2),ceild(4*t1-TS+2,4));
    ubp=min(floord(4*t1+N0,8),floord(N0+2*TS-5,8));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(0,ceild(t1-1,2)),ceild(8*t2-N0-1,8));t3<=min(min(min(floord(N1+2*TS-5,8),floord(4*t1+N1+3,8)),floord(8*t2+N1+1,8)),floord(8*t1-8*t2+N1+N0+1,8));t3++) {
        for (t4=max(max(max(0,ceild(t1-15,16)),ceild(8*t2-N0-57,64)),ceild(8*t3-N1-57,64));t4<=min(min(min(min(floord(N2+2*TS-5,64),floord(4*t1+N2+3,64)),floord(8*t2+N2+1,64)),floord(8*t3+N2+1,64)),floord(8*t1-8*t2+N2+N0+1,64));t4++) {
          for (t5=max(max(max(max(max(0,ceild(8*t2-N0+3,2)),ceild(8*t3-N1+3,2)),ceild(64*t4-N2+3,2)),2*t1),4*t1-4*t2+1);t5<=min(min(min(min(min(floord(8*t1-8*t2+N0+4,2),TS-1),2*t1+3),4*t2+2),4*t3+2),32*t4+30);t5++) {
            for (t6=max(max(8*t2,2*t5+2),-8*t1+8*t2+4*t5-7);t6<=min(min(8*t2+7,-8*t1+8*t2+4*t5),2*t5+N0-3);t6++) {
