...
if ((N0 >= 5) && (N1 >= 5) && (N2 >= 5) && (TS >= 1)) {
  for (t1=0;t1<=floord(TS-1,8);t1++) {
    for (t2=2*t1;t2<=min(floord(N0+2*TS-5,8),floord(16*t1+N0+11,8));t2++) {
      for (t3=max(ceild(8*t2-N0-1,8),2*t1);t3<=min(min(floord(N1+2*TS-5,8),floord(16*t1+N1+11,8)),floord(8*t2+N1+1,8));t3++) {
        for (t4=max(max(ceild(t1-3,4),ceild(8*t2-N0-57,64)),ceild(8*t3-N1-57,64));t4<=min(min(min(floord(N2+2*TS-5,64),floord(16*t1+N2+11,64)),floord(8*t2+N2+1,64)),floord(8*t3+N2+1,64));t4++) {
          for (t5=max(max(max(ceild(8*t2-N0+3,2),ceild(8*t3-N1+3,2)),ceild(64*t4-N2+3,2)),8*t1);t5<=min(min(min(min(TS-1,8*t1+7),4*t2+2),4*t3+2),32*t4+30);t5++) {
